...
import { MessageSquare, Clock, User, Settings } from 'lucide-react';
...
export default function DoctorChatList() {
  ...
  const loadConversations = async () => {
    ...
    try {
      ...
      const { data: conversationsData, error: conversationsError } = await supabase
        ...
        .select(`
          *,
          patient:profiles!chat_conversations_patient_id_fkey(first_name, last_name, role)
        `)
      ...
      const conversationsWithMessages = await Promise.all(
        (conversationsData || []).map(async (conv) => {
          ...
          const { count: unreadCount } = await supabase
            ...
            .eq('conversation_id', conv.id)
            .eq('sender_id', conv.patient_id)
            .eq('is_read', false);

          return {
            ...conv,
            patient_name: `${conv.patient?.first_name} ${conv.patient?.last_name}`,
  ...
  const toggleChatEnabled = async (conversationId: string, enabled: boolean) => {
    try {
      const { error } = await supabase
        .from('chat_conversations')
        .update({ doctor_chat_enabled: enabled })
        .eq('id', conversationId);

      if (error) throw error;

      // Find the conversation to get patient info
      const conversation = conversations.find(c => c.id === conversationId);
      if (conversation && enabled) {
        // Create notification for patient when chat is enabled
        await supabase.from('notifications').insert({
          user_id: conversation.patient_id,
          title: 'Chat Enabled',
          message: `Dr. ${profile?.first_name} ${profile?.last_name} has enabled chat for your appointment. You can now send messages.`,
          type: 'success',
      ...
      toast({
        description: enabled ? 'Chat enabled for patient' : 'Chat disabled for patient'
      });
    } catch (error) {
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    <div className="flex items-center space-x-2" onClick={(e) => e.stopPropagation()}>
                      <span className="text-xs text-gray-500">Enable Chat</span>
                      <Switch
                        checked={conversation.doctor_chat_enabled}
                        onCheckedChange={(enabled) => toggleChatEnabled(conversation.id, enabled)}
                        size="sm"
                      />
                    </div>
                  </div>
